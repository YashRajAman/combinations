...
class main():
    def __init__(self):
        ...
        self.count_button = Button(self.root, text="Just Count Words", command=lambda: self.loops(2))
        self.count_button.pack(side="right")
        self.show_button = Button(self.root, text="Show Words", command=lambda: self.loops(1))
    ...
    def combinations(self, wordlist, buttonid):
        wordlist_in = []
            ...
            for j in range(len(self.wordlist)):
                if((self.word[i]+self.wordlist[j]) not in self.full_list):
                    ...
                    if(buttonid==1):
                        ...
                        self.scrolltext.delete(1.0, END)
                        self.scrolltext.insert(tk.END, str(self.count)+" words")
                else:
                    pass
                self.count_all += 1
                j +- 1
    ...
    def loops(self, button_id):   ##using button_id to check the pressed button.
        k = 0
        self.get_input()
        self.scrolltext.delete(1.0, END)
        while(k<(len(self.word)-1)):
            wordlist_out = self.combinations(self.wordlist, button_id)
